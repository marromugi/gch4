...
- 面談後のレビュー用チャットを開始できる
- 企業方針で定義した高優先観点に沿って LLM が質問を行う
- 面接官は特別フォームなしで自由回答できる
- 面接官回答を `事実 / 解釈 / 不確実性 / 追加確認事項` に構造化して保存できる
- 出力は候補者への返却ではなく、社内 HR 向け共有に利用する
...
- 面談後レビューで使う質問軸を、企業ごとの方針として定義する
- LLM は「評価軸の草案作成」を支援し、最終確定は HR が行う
- 採否の自動判定やスコアリングのルールは作らない

### 全体フロー

1. HR が求人の基本情報（職種、期待役割、カルチャー背景）を入力する
2. 方針作成エージェントが HR へヒアリング質問を行う
3. 回答から観察シグナル候補を抽出し、`Must / NG / Nice` に分類する
4. `Must` と `NG` を中心に `高優先観点` 候補を 3〜5 件提示する
5. HR が候補を採択・修正して方針を確定する
6. 確定した方針をレビューエージェント参照用に保存する

### ヒアリング質問（初期セット）

1. この面談で絶対に確認したいことを 3 つ挙げてください
2. 見送り判断につながる懸念シグナルは何ですか
3. カルチャー上、合わないと困る行動は何ですか
4. それぞれ面談中に観察できる具体例は何ですか
5. 今回確認できなかった場合、次回何を追加確認すべきですか

### 高優先観点の選定ルール

- 面談中に観察可能な行動・発言に限定する
- 1 観点 1 意味を守る（複数観点の混在を避ける）
- 観点数は 3〜5 件に制限し、運用時の認知負荷を抑える
- `Nice` は補助観点として保持し、高優先観点には含めない
- LLM 提案は必ず HR が承認して確定する

### 方針データモデル（例）

- `policy_id`: 方針識別子
- `job_id`: 紐づく求人 ID
- `high_priority_signals`: 高優先観点（3〜5 件）
- `supporting_signals`: 補助観点（任意）
- `concern_signals`: 懸念シグナル
- `prohibited_topics`: レビューで扱わない話題
- `version`: 方針バージョン

### ガードレール

- 業務関連性のない個人属性は方針に含めない
- 抽象語のみの観点は観察可能な表現へ言い換える
- 企業方針と無関係な質問テンプレートを固定適用しない

### イベント計測

- `policy_draft_started`: 方針作成開始
- `policy_draft_confirmed`: HR が方針草案を確定
- `policy_version_published`: 方針バージョンを公開

## 面談レビューエージェント思考フロー

### 目的

- 面接官の自由回答を、社内 HR が再利用しやすいレビュー形式へ標準化する
- 候補者へのフィードバック生成は行わない
- 採否判定やスコアリングは行わない

### 全体フロー

1. 面接官が面談後レビュー チャットを開始する
2. レビューエージェントが `high_priority_signals` を読み込む
3. まずオープン質問で全体所感を自由回答してもらう
4. 回答から `事実` を抽出し、観点ごとに紐づける
5. `unclear` または `not_observed` の高優先観点だけを追加質問する
6. `soft_cap` 到達後は未充足の高優先観点のみを優先する
7. `hard_cap` 到達時は打ち切り、未確認事項を明示して要約を作成する
8. 面接官が要約を確認・微修正し、HR 向けレビューとして保存する

### 観点ステータス定義

- `captured`: 根拠事実と解釈が揃っている
- `unclear`: 事実または解釈の確度が不足している
- `not_observed`: 面談内で確認できなかった

### 出力スキーマ

- `evidence_facts`: 観察した発言・行動の事実
- `interpretations`: 事実に基づく解釈
- `uncertainty`: 解釈の揺らぎと判断保留理由
- `followup_items`: 次回確認事項
- `signal_status`: 観点ごとの `captured / unclear / not_observed`

### 質問生成ルール

- 初回はオープン質問を 1 つだけ行い、自由回答を優先する
- 追加質問は不足観点の補完に限定する
- 1 質問 1 観点を守り、重複質問を避ける
- 既出事実の再確認が必要な場合は理由を明示する
- 十分な情報が揃った時点で要約確認へ進む

### 解釈の揺らぎ対策

- 各解釈は最低 1 つの `evidence_fact` に紐づける
- 各解釈に `confidence`（`high / medium / low`）を付与する
- 各高優先観点で `counter_interpretation` を 1 つ以上記録する
- `low` が多い場合は `followup_items` の出力を必須にする

### レビュー時の禁止トピック

- 年齢、性別、性的指向、人種・民族、宗教、政治信条
- 婚姻状況、妊娠・出産計画、家族構成、介護状況
- 健康情報、障害、病歴などのセンシティブ情報
- 業務と無関係な私生活情報
- 前職・現職の機密情報の開示誘導

### フォールバック

- `hard_cap` 到達時に `unclear` が残る場合、未確認項目を一覧表示する
- 面接官に未確認項目を手入力してもらい、状態を再判定する
- 入力完了後にレビューを保存し、`review_manual_fallback_triggered` を記録する

### イベント計測

- `review_chat_started`: 面談後レビュー開始
- `review_turn_soft_capped`: soft cap 到達
- `review_turn_hard_capped`: hard cap 到達
- `review_summary_confirmed`: 面接官が要約確認を完了
- `review_submitted`: HR 向けレビュー保存完了
- `review_manual_fallback_triggered`: 面談後レビュー手入力フォールバック発動
...
- 面談後レビュー チャットを実装する
  - 自由回答 + 不足観点の追加質問で面接官回答を構造化して保存する
- データ保護要件の運用補強を行う
  - 削除依頼の受付導線
  - 閲覧権限の分離（必要な人のみ）
