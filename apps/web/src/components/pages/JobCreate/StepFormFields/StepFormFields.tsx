...
export function StepFormFields() {
  ...
  const { control, formState, watch } = useFormContext<JobCreateFormValues>()
  ...
  const idealCandidate = watch('idealCandidate')
  const cultureContext = watch('cultureContext')

  const { suggest, formFields: suggestedFields, isLoading, error } = useFormFieldsSuggest()

  const handleSuggest = () => {
    if (!title || isLoading) return
    suggest(title, idealCandidate, cultureContext)
  }
