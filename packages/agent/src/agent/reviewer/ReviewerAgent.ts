 ...
 * - 情報の明確性
 */
export class ReviewerAgent extends BaseAgent<ReviewerContext, ReviewerTurnResult> {
   ...
   * 1ターンの実行
   */
  async executeTurn(context: ReviewerContext, _userMessage: string): Promise<ReviewerTurnResult> {
    this.logger.info('Starting Reviewer', {
      fieldId: context.currentField.fieldId,
      answerLength: context.collectedAnswer.length,
    })
    ...
    const messages = [...context.chatHistory, { role: 'user' as const, content: reviewPrompt }]
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      ...
      if (attempt > 0 && lastResponse) {
        ...
        messages.push({
          role: 'user' as const,
          content: this.buildRetryPrompt(attempt, context.collectedAnswer),
        })
      }
      ...
      if (reviewCall) {
        ...
        const reviewResult = result.result as ReviewResult

        this.logger.info('Reviewer completed', {
          passed: reviewResult.passed,
          fieldValue: reviewResult.fieldValue,
